...
// counters plus a few gauges, no external dependency (ADR-0001). Secret material
// never appears here, and caller-supplied strings are never used as label values
...
type Metrics struct {
	...
	gauges   map[string]map[string]float64 // gauge name -> rendered labels -> value
...
func NewMetrics() *Metrics {
	return &Metrics{
		...
		help: map[string]string{
			...
			"acb_connect_token_expiry_timestamp_seconds": "Unix expiry of the 1Password Connect token (the broker's one long-lived credential); 0/absent if unparseable.",
		},
	}
}
...
// Set writes a gauge (last-write-wins), for point-in-time facts like a
// credential's expiry timestamp. Same cardinality discipline as counters.
func (m *Metrics) Set(name string, labels map[string]string, v float64) {
	m.mu.Lock()
	defer m.mu.Unlock()
...
func (m *Metrics) ServeHTTP(w http.ResponseWriter, _ *http.Request) {
	...
	m.mu.Lock()
	render := func(store map[string]map[string]float64, typ string) {
		...
		for _, n := range names {
			...
			series := make([]string, 0, len(store[n]))
			for ls := range store[n] {
	...
	render(m.counters, "counter")
	render(m.gauges, "gauge")
	m.mu.Unlock()
